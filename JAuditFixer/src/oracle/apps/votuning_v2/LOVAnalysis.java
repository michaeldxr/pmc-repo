...
public class LOVAnalysis {
    ...
    public boolean hasPanelStretchLayoutAnscestor(Node x)
    {
        Node p = x.getParentNode();
        ...
        if(p.getNodeName().equals("af:panelStretchLayout"))
        {
            // disqualify panelStretchLayou cases that are dimensionsFrom = children
            NamedNodeMap nnm = p.getAttributes();
            Node df = nnm.getNamedItem("dimensionsFrom");
        ...
        return hasPanelStretchLayoutAnscestor(p);
    }

    public String getPanelStretchLayoutAnscestor(Node x)
    {
        Node p = x.getParentNode();
        ...
        if(p.getNodeName().equals("af:panelStretchLayout"))
            return p.getNodeName() +" -- " +x.getNodeName();
        
        String psl_anscestor = getPanelStretchLayoutAnscestor(p);
        if(psl_anscestor == null || "".equals(psl_anscestor))
            return "";

        return psl_anscestor +" -- " +x.getNodeName();
    }
    ...
    public void traverseNodes(NodeList childNodes, File uiFile) {
        ...
        for (int i = 0; i < childNodeLength; i++) {
            ...
            if (!isEmpty(nodeName) && map_component_iterators.containsKey(nodeName)) 
            {
                TuningAnalysisRecord record = new TuningAnalysisRecord();
                
                if("af:table".equals(nodeName) || "af:treeTable".equals(nodeName))
                {
                    boolean flag1 = isDirectChildOfPanelStretchLayout(child);
                    boolean flag2 = hasPanelStretchLayoutAnscestor(child);

                    String ancestry = getPanelStretchLayoutAnscestor(child);

                    if(flag1 != flag2)
                    {
                        System.out.println("********************************************************");
                        System.out.println("interesting case in file:");
                        System.out.println(uiFile.getAbsolutePath());
                        System.out.println(getPanelStretchLayoutAnscestor(child));
                        System.out.println("********************************************************");
                        
                        ancestry = ancestry + " need xcheck ancestry logic";
                    }

                    record.setTableWrappedByPanelStretchLayout(flag1);
                    record.setPanelStretchAncestry(ancestry);
                }
