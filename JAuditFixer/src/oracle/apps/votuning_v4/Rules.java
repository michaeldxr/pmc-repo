...
import oracle.apps.votuning_v4.ScanResult;
...
public class Rules {
  ...
  public static boolean validateLOVs(ScanResult result) {
    ...
    //ignore transient vo as per bug 13958137
      if(result.isIsTransientVO()){
        return true;
      }
...
//    }
  ...
  public static boolean validateTables2ndRun(ScanResult result) {
    ...
    try{
      ...
      //Check Rule 2 only if the vo is non-transient VO. bug-13958137
        if(!result.isIsTransientVO())
        {
      ...
      //    }
        ...
        //Fix autoHeightRows only if this record is a violation already.
        if(isViolation){
            if (result.getIsWrappedByPanelStretchLayout().equals("Yes") &&
                result.getEffAutoHeightRows() != null &&
                Integer.parseInt(result.getEffAutoHeightRows()) < 15) 
            {
                description += "TableFetchSizePSL. ";
//                result.setNew_autoHeight("-1");
//                isViolation = true;
            }
        }
    }catch(Exception e){
