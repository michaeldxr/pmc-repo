...
import oracle.apps.votuning_v4.file_type.Application;
import oracle.apps.votuning_v4.file_type.Project;
...
public class JarAnalyzerPMC {
  ...
  public static void process(String[] files, String adeViewRoot, String label, String family){
    ...
    if (hs.size() == 0) {
      System.out.println("Neither UI nor xml files found in the txn. Skipping VOTuning_V3 check");
      return;
    }
    ...
    try {
      ...
      if (releaseNum <
          11) //This check was mandated post release 11 only, hence this check is necessary to avoid the current rel9 and rel10 branches from failing
        return;
    ...
    if (label.contains("FSM")) {
      ...
      analyzer.crawlDirectory(path,MAX_DEPTH);
    } else {
      for (String f : h_family) {
        String path = adeViewRoot + "/fusionapps/" + f + "/components";
        System.out.println("Processing " + path);
        analyzer.crawlDirectory(path,MAX_DEPTH);
      }
    }
    ...
    ScanResultsAnalyzer resultAnalyzer =
      new ScanResultsAnalyzer(Project.adePathMapping);
    resultAnalyzer.AnalyzeResult();

    ArrayList<ScanResult> al_violations =
      finder.validate(resultAnalyzer.al_results);
  ...
  private void crawlDirectory(String path, int maxDepth) {
    if(maxDepth == 0)
      return;
    ...
    if (path.contains(".ade_path") || path.contains("/classes/") ||
        path.toLowerCase().contains("soa") ||
        path.toLowerCase().contains("ess"))
      return;
    ...
    for (int i = 0; i < listOfFiles.length; i++) {
      if (listOfFiles[i].isFile()) {
        ...
        if (isDirOfInterest(listOfFiles[i].getAbsolutePath()))
          crawlDirectory(listOfFiles[i].getAbsolutePath(), maxDepth);
      } else
  ...
  private boolean isDirOfInterest(String sName) {
    if (sName.contains(".ade_path") || sName.contains("/classes/") ||
        sName.toLowerCase().contains("soa") ||
        sName.toLowerCase().contains("ess"))
      return false;
  ...
  private ArrayList<ScanResult> isAffectedFiles(ArrayList<ScanResult> al_violations,
    ...
    while (i.hasNext()) {
      r = i.next();
      if (files.contains(r.getPage()) || files.contains(r.getPageDef()) ||
          files.contains(r.getAM()) || files.contains(r.getVO()) ||
          files.contains(r.getVa_VO())) {
        continue;
      } else {
        i.remove();
      }
  ...
  private static HashSet<String> filterAffectedFiles(String[] files,
                                                     String adeViewRoot) {
  ...
  private static void writeToFile(ArrayList<ScanResult> al_violations) throws Exception {
    ...
    writer.write("\n\nPlease see http://globaldc.oracle.com/perl/twiki/view/FusionSharedTools/TableAndLOVTuningV3" +
