...
public class JarAnalyzerLRG{
  ...
  public static void main(String[] args) {
      ...
      if(!series.contains("FSM")){
          ...
          if(releaseNum < 11){
              System.out.println("WARNING: you are trying to run votuning on series " +series + " , which is not a Release 11+ branch. Please run votuning_v2 for R9 and R10.");
              System.exit(1);
          }
      }
    ...
    System.out.println("Running TableAndLOVTuning_v3 Scan on series: " + series + ".......");
    
    ArrayList<String> families = LRGUtil.getFamiliesFromSeries(series);
    for (int j = 0; j < families.size(); j++) {
        String family = families.get(j);
        String sCrawlDir = LRGUtil.getCrawlDir(series, family, label, "TableAndLOVTuning_v3");
    ...
    LRGUtil.addLabelInfoToAB("TableAndLOVTuning_v3", series, label);   
    FamilyModuleHelper. printScanDuration(start,"VoTuning");
  }
