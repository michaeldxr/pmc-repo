...
public class ViewCriteriaUsedInUInew extends JoesBaseClass {
    ...
    static HashSet<String> exemptedTables = new HashSet<String>();
    static HashSet<String> exemptions1 = new HashSet<String>(); //UIsearches3
    ...
    static HashSet<String> vosAlreadyProcessed = new HashSet<String>(); //contains jsff
    static HashSet<String> vosAlreadyProcessed1 = new HashSet<String>(); //vo + vc..no jsff
    
    static HashMap<String,ArrayList<String>> vovcViolations = new HashMap<String,ArrayList<String>>();
   // static HashMap<String,HashSet<String>> vosProcessedJsffs = new HashMap<String,HashSet<String>>();
    ...
    private static int releaseNum=13;
    
//    static BufferedWriter lovVoWriter;
//    static BufferedWriter lovVoWriter1;
    static HashMap<String,String> LovVOToOrigVoMapping = new HashMap<String,String>();
    
    public ViewCriteriaUsedInUInew() 
    {
          ...
          for (int i = 0; i < searchRegionList.getLength(); i++) 
          {
              ...
              try{                
               ...
               //  get Binds attr value and get the iterator from the iterator map
                  ...
                  if(amAbsPath == null){
                    System.out.println("AM path is null: " + dcPackage + "." + amName);
                    return;
                  }
    ...
    public void processLovVO(XMLDocument voXml, String voFileName, String modelJprLocation, String lovUses, String jsff, String modelValue, String component) throws Exception {
        ...
        //lovVoWriter.write(voFileName + "," + lovUses + "," + jsff + "," + modelValue + "," + component + "\n");
        NodeList listViewObj = voXml.getElementsByTagName("ViewObject");
        if (!XMLParserHelper.hasOneNOnlyOneNode(listViewObj)) 
        ...
        //lovVoWriter1.write(bcPath+","+viewCriteriaUsed + " --> " + voFileName + "," + lovUses+"\n");
        LovVOToOrigVoMapping.put(ViewCriteriaHelper.getFileName(bcPath)+";"+viewCriteriaUsed,ViewCriteriaHelper.getFileName(voFileName) + ";" + lovUses);
        
        String voProcessedKey = bcPath+viewCriteriaUsed;
       
        //voProcessedKey += jsff;
        
        if(vosAlreadyProcessed.contains(voProcessedKey+jsff)){
           // lovVoWriter.write("Already processed vo.vc.jsff: " + voProcessedKey + jsff + "\n");
           return; //nothing else to do
        }
        else 
            vosAlreadyProcessed.add(voProcessedKey+jsff);
        
        if(vosAlreadyProcessed1.contains(voProcessedKey)) {
            
           // lovVoWriter.write("Getting Already processed violations for vo.vc: " + voProcessedKey + "\n");
           ArrayList<String> violations = vovcViolations.get(voProcessedKey);
           if(violations != null){
               for(int i = 0; i < violations.size(); i++){
                   String violation = violations.get(i);
                   writer.write(ViewCriteriaHelper.getFileNameInfo(jsff,null) + component + "," + modelValue + "," + ViewCriteriaHelper.getFileName(bcPath)
                       + "," +  viewCriteriaUsed + "," + violation + "\n");
               }
           }
            return;
        }
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        ViewCriteriaUsedInUInew x = new ViewCriteriaUsedInUInew();
        x.generateExemptionList(); 
        
        writer = new BufferedWriter(new FileWriter("UI_Searches.csv"));                 
        ...
        if(mode.equals("view")){
            ...
            //sCrawlDir = "/ade_autofs/ud21_fa/FUSIONAPPS_PT.R9INT_LINUX.X64.rdd/LATEST/fusionapps/fin/components/";
          ...
          //String series = "FUSIONAPPS_PT.V2MIB_LINUX.X64";
            ...
            System.out.println("Running UISearchCaseSensitivity_new Scan on series: "+series+".......");
          ...
          String viewRoot = serverTop + "/"+series+".rdd/"+label + "/";
            ...
            for(int j=0; j<families.size();j++){
                ...
                //x.clearDataStructures();//do not clear data sturcutres..processFiles() is called once in the end.
                String family = families.get(j);
                m_family = family;
                String sCrawlDir = LRGUtil.getCrawlDirNew(series, family, "UISearchCaseSensitivity_new");       
            ...
            x.processFiles();
            LRGUtil.addLabelInfoToAB("UISearchCaseSensitivity_new", series, label);
        }
        ...
        while((line=reader.readLine()) != null) {
            ...
            if(parts.length > 10){
                ...
                String origVOLOVUses = LovVOToOrigVoMapping.get(lovVo+";"+vcUsed);
        ...
        if(mode.equals("view") && makeFix){
            System.out.println("Fixing case-sensitive VCIs used in UI that are optional or, corresponding table columns have no/upper indexes");
            x.startDiffLog();     
            x.makeFixes();
            x.closeDiffLog();    
        ...
        FamilyModuleHelper.printScanDuration(start,"UISearches");
    }
...
//           writer.write(ViewCriteriaHelper.getFileNameInfo(jsff,null) + component + "," + model + "," + ViewCriteriaHelper.getFileName(f)
//                        + "," +  criteriaName + ",Implicit VC results in Case-insensitive search, , , , , , , , ,No,Case1:Implicit VC (No Fix)\n");
         return;
        }
    ...
    public void processViewCriteria(XMLDocument voXml, NodeList viewCriteria, String fileName, String modelJprLocation,
        ...
        for(int k = 0; k < vcRows.getLength(); k++ ){
          ...
          for(int j =0; j < vc_row_children.getLength(); j++) {
            ...
            String lovName = XMLParserHelper.getAttributeValue(va, "LOVName");
              ...
              if(!ViewCriteriaHelper.isEmpty(lovName) && component.equals("af:query")){
                lovVosToProcess.add(new LovVO(voXml,fileName, modelJprLocation,lovName,jsff,model,"query panel LOV"));
                continue;
              }
              
              if(isUpper.equals("case-insensitive"))
                  continue; // preempt further processing 
            ...
            if(category.contains("Case6")) 
                continue; //TODO: This should be a part of case-insentitive VC scan..  
            ...
            if(jsff.contains("oracle/apps/hcm/compensation/salary/core/publicUi/page/SalarySearch.jsff")) //Test page by HCM
                continue;
              if(releaseNum <=10 && fileName.contains("payables/invoices/transactions/model/view/ManageInvoiceSupplierPVO.xml") 
                 && description.contains("vcrow574"))
                  continue; //Bug 20708853
              
              if(releaseNum <=10 && fileName.contains("oracle/apps/financials/assets/shared/publicView/AssetSearchPVO.xml") 
                 && description.contains("vcrow522"))
                  continue; //Exemption for Release 9 = Bug 21204613 
              
              if(!exemptions.contains(ViewCriteriaHelper.getFileName(fileName) + "," + description)
                 && !exemptions1.contains(ViewCriteriaHelper.getFileName(fileName) + "," + description) //TODO: Su: exemption1 and isexemptedTable violations are UIsearches3...
                 && !isExemptedTable(table_name + "," + resolvedTableColumn)){
                    ...
                    violations.add(description + "," + viewAttrName + "," + table_name +  "," + col_name + "," +
                         resolvedTableColumn + "," + colType + "," + index + "," + vcRequired + "," 
                         + col_position + "," + caseSensitive + "," + simpleCat);
      ...
      if(violations.size() > 0)
        vovcViolations.put(fileName+criteriaName,violations);
  }
   ...
   public void generateExemptionList() {
      ...
      while((line = fileReader.readLine()) != null) {
       ...
       fileReader.close();     
        ...
        fileReader = new BufferedReader(new FileReader(FamilyModuleHelper.getLabelServer("FATOOLS_MAIN_LINUX") +
            "/fatools/opensource/jauditFixScripts/txt/UISearch_exemptions.txt"));
        line = "";
        ...
        while((line = fileReader.readLine()) != null) {
          ...
          String issue = parts[0].trim();
          if(issue.equals("UISearches"))
              exemptions1.add(parts[1].trim() + "," + parts[2].trim());
    ...
    public void makeFixes() throws Exception {
        ...
        BufferedReader fileReader = new BufferedReader(new FileReader("UI_Searches.csv"));
