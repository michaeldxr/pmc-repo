...
/* Su Notes: This script will find the SmartFilter vioaltions + noViewCriteriaNew and MRU violations along with the old ones.
 * So if this is released in a future release it can replace the old version of the script i.e. ViewCriteriaUIPerformance_enhanced.java
 * This script support the normal view mode for developers as well as LRG mode.
 * To run the script in LRG mode it simply needs to be invoked with -Dmode=LRG from the shell script
 * Zhao can integrate the scan with LRG if this is released. Tell him to not run the older version of the script 
 * i.e. ViewCriteriaUIPerformance_enhanced for branches on which this is running as this supercedes the older version.*/

public class ViewCriteriaUIPerformance_smartFilter extends JoesBaseClass{
    ...
    static HashMap<String,String> voMaxRowsCalculated = new HashMap<String,String>();
    
    boolean bDebug = false;
    static String  sCrawlDir;
    private static String mode="view"; //can be view/LRG/premerge
    private static final String issuetype = "ViewCriteriaUIPerformance_smartFilter";
    ...
    static HashMap<String,ArrayList<String>> reportedViolations = new HashMap<String,ArrayList<String>>(); //without the jsff...
   // static HashMap<String,HashSet<String>> vosProcessedJsffs = new HashMap<String,HashSet<String>>();
    ...
    public ViewCriteriaUIPerformance_smartFilter() 
    {
    ...
    protected void processPage(String f) 
    {
      m_page_reviewCount++;
      if(bDebug)
         System.out.println("Checking Page:  " +f.toString() + " starting at " + new java.util.Date());
        ...
        try {
          ...
          for (int i = 0; i < queryList1.getLength(); i++) 
          {
           ...
           HashSet<Node> lovNodes = new HashSet<Node>();
            ...
            if(pageDef != null && usedQueryModels.size() > 0 || lovNodes.size() >0)
          ...
          for (int i = 0; i < searchRegionList.getLength(); i++) 
          {
              ...
              try{                
               ...
               //  get Binds attr value and get the iterator from the iterator map
                  ...
                  processVO(voXml, bcPath, modelJprLocation, jsff, criteria, id,"query",viewUsageVCs,voProcessedKey,false, null,null, voKey);     
...
//                    System.out.println("break here");
    ...
    public void processLovVO(XMLDocument voXml, String voFileName, String modelJprLocation, String lovUses, String jsff, String modelValue, String component) throws Exception {
        ...
        if(displayCriteriaValue == null)
            displayCriteriaValue="No DisplayViewCriteria";
        ...
        if(vcUsageValue == null)
            vcUsageValue="No ViewCriteriaUsage";
        ...
        if(displayCriteriaValue != null) {
            String voProcessedKey = bcPath+displayCriteriaValue;
            ...
            if(!ViewCriteriaHelper.isEmpty(bcPath)){
                ...
                processVO(voXml, bcPath, modelJprLocation,jsff, displayCriteriaValue, modelValue, component,viewAccessorvcUsages,voProcessedKey,false,voFileName,mruCount,voKey);
            }
        } 
        
        if(vcUsageValue != null && (component.equals("af:inputComboboxListOfValues") || component.equals("query panel combo LOV"))) {
            
             String voProcessedKey = bcPath+vcUsageValue;
            for(int j = 0; j < viewAccessorvcUsages.size(); j++)
                voProcessedKey += viewAccessorvcUsages.get(j);      
            ...
            if(!ViewCriteriaHelper.isEmpty(bcPath)){
                debugWriter.write(bcPath + "," + displayCriteriaValue + "," + modelValue + "," + voFileName + "," + lovUses + "\n");
                processVO(voXml, bcPath, modelJprLocation,jsff, vcUsageValue, modelValue, component,viewAccessorvcUsages,voProcessedKey,true,voFileName,mruCount,voKey);
            }
        } 
    }
    ...
    public void processFiles() 
    {
      // first load the cpx so we have a mapping between pages and pageDefs
        System.out.println("Digesting CPX...");
    ...
    { 
        ...
        ViewCriteriaUIPerformance_smartFilter x = new ViewCriteriaUIPerformance_smartFilter();
        x.generateExemptionList(); 
        writer = new BufferedWriter(new FileWriter("VC_scan_UI_Performance.csv"));   
        debugWriter = new BufferedWriter(new FileWriter("listBindings.txt"));
        //debugWriter.write(bcPath + "," + displayCriteriaValue + "," + modelValue + "," + voFileName + "," + lovUses + "\n");
        debugWriter.write("VO in which violation is defined, criteria name, model value, voFile where LOV is defined, LOV Usage name\n");
        ...
        writer1.write("Family,Module,Product,Filename,Label,ViewName,Issue,Jsff,Model,Component,MRUCount\n");
     ...
     //sCrawlDir = "/ade_autofs/ud21_fa/FUSIONAPPS_PT.V2MIBFIND_LINUX.X64.rdd/LATEST/fusionapps/fin/components/payables/ce/bankRelationships/ui";
            ...
            x.parser = new ModifiedComboboxParser(m_family.toLowerCase(), ModifiedComboboxParser.ScanType.ALL);
            int releaseNo = 11;
            
            String release = "";
            if(args.length > 1){
                String series = args[1].trim();
                release = FamilyModuleHelper.getRelease(series);
            }
                if(!ViewCriteriaHelper.isEmpty(release)) {
                    ...
                    if(releaseNo < 11) {
                        ...
                        System.out.println("Using old indexes...");
              ...
              System.out.println("Release unknown. Resorting to default R11 metadata...");
              function_indexColumns = SqlIndexFinder.readIndexesFromFile_rel11("function");
              default_indexColumns = SqlIndexFinder.readIndexesFromFile_rel11("default");
              SqlIndexFinder.readColPositionsFromFile_rel11();
              SqlIndexFinder.readTableDataFromFile_rel11();
...
//            families.add("fin");
            ...
            viewRoot = serverTop + "/"+series+".rdd/"+label + "/";
            for(int j=0; j<families.size();j++){
                ...
                x.parser = new ModifiedComboboxParser(family, ModifiedComboboxParser.ScanType.ALL);
        ...
        writer1.write("Family,Module,Product,Filename,Label,ViewName,Issue," +
                "UI File,Model,Component, MRUCount, ListOfTables, MaxRows, Blocks, TableWithMaxRows,WHEREContainsBinds\n");
        ...
        while((line = reader.readLine()) != null){
                ...
                if(!exemptions.contains("NoViewCriteriaLOV,"+lovVOToBaseVOMapping.get(key) + "," + parts[8].trim() + "," + parts[7].trim()))
                    ViewCriteriaHelper.getMaxRows(fileName,writer1,line,voXml, noVCexemptedTables1, noVCexemptedTables2_maxrows,lovVOToBaseVOMapping,voMaxRowsCalculated);
            }
            
        writer1.close();
        FamilyModuleHelper.printScanDuration(start,"ViewCriteriaUIPerformance");
    }
        
    
    
    public void processVO(XMLDocument doc, String f, String modelJprLocation,String jsff, String criteriaName, 
                          String model, String component, ArrayList<String> viewUsageVCs, String voProcessedKey, 
                          boolean smartFilter, String origVOName, String mruCount, String voKey) throws Exception {    
        ...
        if(reportedViolations.containsKey(voKey)){
            
            ArrayList<String> violations = reportedViolations.get(voKey);
            for(int i = 0; i < violations.size(); i++) {
                String violation = violations.get(i);
                String[] parts = violation.split(",");
                if(violation.contains(",VO has no") && parts.length >= 10)
                    writer1.write(violation.replace(parts[7],FamilyModuleHelper.getPathAfterViewRoot(jsff)));
                else if(parts.length > 22)
                    writer.write(violation.replace(parts[22],FamilyModuleHelper.getPathAfterViewRoot(jsff)));                
            }
            return;
        }
        ...
        if(viewCriteria == null || viewCriteria.getLength() == 0)   {
            ...
            if(!exemptions.contains("NoViewCriteria,"+ViewCriteriaHelper.getFileName(f,jsff) + "," + model + "," + ViewCriteriaHelper.getFileName(jsff))){
                ...
                if(!smartFilter){
                    violation = ViewCriteriaHelper.getFileNameInfo(f,jsff) + ViewCriteriaHelper.getViewName(f) +
                   ",VO has no ViewCriteria (" + criteriaName + ")," + ViewCriteriaHelper.getFileName(jsff) + "," + model + "," + component  + "," + mruCount + "\n";
                    
                    writer1.write(violation);
                    
                    ArrayList<String> violations = new ArrayList<String>();
                    if(reportedViolations.containsKey(voKey))
                        violations = reportedViolations.get(voKey);
                    violations.add(violation);
                    reportedViolations.put(voKey,violations);
                }
//TODO: uncomment when smartFilter is approved...refactor to add to reportedViolations
//                else
//                    writer1.write(ViewCriteriaHelper.getFileNameInfo(f,jsff) + ViewCriteriaHelper.getViewName(f) +
//                    ",VO has no ViewCriteria (LOVSmartFilterNoViewCriteria)," + ViewCriteriaHelper.getFileName(jsff) + "," + model + "," + component + "," + mruCount + "\n");
                   
            }         
            lovVOToBaseVOMapping.put(ViewCriteriaHelper.getFileName(f)+"," + ViewCriteriaHelper.getFileName(jsff) + "," + model, ViewCriteriaHelper.getFileName(origVOName)); 
            return;
        }
        ...
        if(reportedViolations.containsKey(voKey)){
            ...
            for(int i = 0; i < viewUsageVCs.size(); i++){
                passed = processViewCriteria(doc, viewCriteria, f, modelJprLocation,
                                             jsff, viewUsageVCs.get(i), model,component,false, voProcessedKey, smartFilter, voKey);
            ...
            if(vcPassed)
                return; //skip the check - at least one of the ANDed VCs passed the badVC Item logic.
            //TODO: Uncomment  viewcritriausagew when smart filter is approved
            if(criteriaName.toLowerCase().contains("implicitviewcriteria") || criteriaName.toLowerCase().contains("displayviewcriteria") /*|| criteriaName.toLowerCase().contains("viewcriteriausage")*/) { 
                
                if(!exemptions.contains("NoViewCriteria,"+ViewCriteriaHelper.getFileName(f,jsff) + "," + model + "," + ViewCriteriaHelper.getFileName(jsff))){
                    
                    if((criteriaName.toLowerCase().contains("displayviewcriteria") || criteriaName.toLowerCase().contains("viewcriteriausage")) && !ViewCriteriaHelper.isEmpty(origVOName)){
                        
                        violation = ViewCriteriaHelper.getFileNameInfo(f,jsff) + ViewCriteriaHelper.getViewName(f) +
                      ",VO has no Required/Selectively Required ViewCriteria (" + criteriaName + ")," + ViewCriteriaHelper.getFileName(jsff)
                            + "," + model + "," + component + "," + mruCount + "\n";
            ...
            if(!criteriaName.contains("pageFlowScope")){
                ...
                processViewCriteria(doc, viewCriteria, f, modelJprLocation, jsff, criteriaName, model,component,true,voProcessedKey, smartFilter, voKey);
...
//TODO: uncomment when smartfilter is approved ...refactor to add to reportedVioaltions.
//                      else
//                          writer1.write(ViewCriteriaHelper.getFileNameInfo(f,jsff) + ViewCriteriaHelper.getViewName(f) +
//                              ",VO has no Required/Selectively Required ViewCriteria (" + criteriaName + "(LOVSmartFilterNoViewCriteria))," + ViewCriteriaHelper.getFileName(jsff) 
//                                + "," + model + "," + component + "," + mruCount + "\n");                
            ...
            for(int i = 0; i < showInListVCsNotAlreadyProcessed.size(); i++) {
                ...
                processViewCriteria(doc, viewCriteria, f, modelJprLocation, jsff, criteria, model,component,true,voProcessedKey, smartFilter, voKey);
                ...
                if(!reqdVCFound && !reqdVCFound1) {
                    ...
                    //TODO: Uncomment when smartfilter is approved
//                    else
//                        writer1.write(ViewCriteriaHelper.getFileNameInfo(f,jsff) + ViewCriteriaHelper.getViewName(f) +
//                        ",VO has no Required/Selectively Required ViewCriteria(savedSearch: " + criteria + "(LOVSmartFilterNoViewCriteria))," + ViewCriteriaHelper.getFileName(jsff) 
//                              + "," + model + "," + component + "," + mruCount + "\n");
                    }
                    if(f.startsWith("oracle/"))
                        voXmlsFromOtherJars.put(f, doc);
    ...
    public String processViewCriteria(XMLDocument voXml, NodeList viewCriteria, String fileName, String modelJprLocation,
                   String jsff, String criteriaName, String model, String component, boolean toWrite, String voProcessedKey, boolean smartFilter, String voKey) throws Exception {
...
//                 
            ...
            String lovName = XMLParserHelper.getAttributeValue(va, "LOVName");
              ...
              if(!ViewCriteriaHelper.isEmpty(lovName) && component.equals("query")){
                  ...
                  boolean comboLOV=false;
                  Node properties = XMLParserHelper.getChildNodeWithName(va, "Properties");
                  if(properties != null) {
                      Node schemaproperties = XMLParserHelper.getChildNodeWithName(properties, "SchemaBasedProperties");
                      if(schemaproperties != null) {
                          Node controlType = XMLParserHelper.getChildNodeWithName(schemaproperties, "CONTROLTYPE");
                          if(controlType != null){
                              String controlTypeValue = XMLParserHelper.getAttributeValue(controlType, "Value");
                              if(controlTypeValue != null && controlTypeValue.equals("combo_lov"))
                                  comboLOV=true;
                          }
                      }
                  }
                  if(!comboLOV)
                    ...
                    lovVosToProcess.add(new LovVO(voXml,fileName, modelJprLocation,lovName,jsff,model,"query panel combo LOV"));
              }
              ...
              if(resultParts.length < 21)
                ...
                if(processingSavedSearch && exemptionsOnVO.contains("SavedSearchBadViewCriteriaItems,"+resultParts[3].trim() + "," + resultParts[6].trim()))
                  ...
                  if(!processingSavedSearch){
                       if(!smartFilter){
                          String violation = result + "," + vcri + "," + ViewCriteriaHelper.getFileName(jsff) + "," + 
                      ...
                      //TODO: write out smart filter violations when it is approved.
//                    if(smartFilter)
//                        writer.write(",LOVSmartFilterBadViewCriteriaItem\n");
//                    else
//                        writer.write("\n");
                  }
                  else{
                      String violation = result + "," + vcri + "(" + criteriaName + ")," + ViewCriteriaHelper.getFileName(jsff) + "," + 
    ...
    private static String getIndex(String tableName, String colName) throws Exception {
       ...
       for(int i = 0; i < tableNames.length; i++) {
          ...
          if(SqlIndexFinder.synonyms.containsKey(tablNameToUse))
           ...
           if(tablNameToUse != null && function_indexColumns != null){
          ...
          if(tablNameToUse != null && default_indexColumns != null){
 ...
 public void generateExemptionList() {
    ...
    //BufferedReader fileReader = new BufferedReader(new FileReader("/scratch/sudgupta/view_storage/sudgupta_fatools_main_vie_may1/fatools/opensource/jauditFixScripts/txt/rup2Exemptions.txt")); //TODO
      ...
      while((line = fileReader.readLine()) != null) {
        ...
        if(issue.equals("NoViewCriteria") || issue.equals("BadViewCriteriaItems") || issue.equals("NoViewCriteriaLOV"))           
