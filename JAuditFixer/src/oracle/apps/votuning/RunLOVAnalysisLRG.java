...
public class RunLOVAnalysisLRG {

    private static String ERROR_LOG_FILENAME = "error.log";
    private static Logger errorLogger = null;
    private static int m_jsffCount = 0;
    private static int m_jspxCount = 0;
    private static HashMap<String, DataBindings> bindingsMap = new HashMap<String, DataBindings>();

    private static String m_diffFileName = "diff.txt";
    private AdeDiffHelper diffHelper = new AdeDiffHelper(m_diffFileName);
    private static Workbook m_myBook;
    private static Worksheet m_analysisSheet;
    private static BufferedWriter writer;

    private static HashMap<String, ArrayList<TuningAnalysisRecord>> allfin_errorRecords = new HashMap<String, ArrayList<TuningAnalysisRecord>>();
    private static HashMap<String, ArrayList<TuningAnalysisRecord>> onlyrequired_errorRecords = new HashMap<String, ArrayList<TuningAnalysisRecord>>();
    private static HashMap<String, ArrayList<TuningAnalysisRecord>> allfin_analysisRecords = new HashMap<String, ArrayList<TuningAnalysisRecord>>();
    private static HashMap<String, ArrayList<TuningAnalysisRecord>> onlyreqiured_analysisRecords = new HashMap<String, ArrayList<TuningAnalysisRecord>>();
    private static HashMap<String, ArrayList<TuningAnalysisRecord>> onlyreq_multipleBcs_fixedrecords = new HashMap<String, ArrayList<TuningAnalysisRecord>>();
    private static HashMap<String, ArrayList<TuningAnalysisRecord>> multipleBcsFinalMap = new HashMap<String, ArrayList<TuningAnalysisRecord>>();
        
    private HashMap<String, ArrayList<TuningAnalysisRecord>> m_map_bcs_records = new HashMap<String, ArrayList<TuningAnalysisRecord>>();
    private HashMap<String, ArrayList<TuningAnalysisRecord>> m_map_violatingBcs_records = new HashMap<String, ArrayList<TuningAnalysisRecord>>();
    private HashMap<String, ArrayList<TuningAnalysisRecord>> m_mapOfMultipleIteratorUsages = new HashMap<String, ArrayList<TuningAnalysisRecord>>();
    private HashMap<String, Integer> defaultVOFetchSizesFixed = new HashMap<String, Integer>();

    private static HashMap<String, Integer> m_map_componentCnt = new HashMap<String, Integer>();
    
    private static boolean bDebug = false;
    private static boolean fix_MultipleBcUsages = false;
    private static boolean fix_selectOneChoice_listRangeSize = false;
    private static boolean fix_combobox_listRangeSize = false;
    private static boolean fix_tune_tables = false;
    private static boolean fix_tune_combobx = false;
    private static boolean fix_tune_selectoneChoice = false;
    private static boolean m_fixlov_only_if_max_exceeded = false;
    private static boolean m_fixViolations = true; 
    private static boolean bDoADE = false;
    private static boolean append = false;
    private static Integer m_fixlov_Max = 250;
        
    private final static String issuetype = "TableAndLOVTuning";
    ...
    public static String getRunningWithParams()
    {
        StringBuffer opts = new StringBuffer();
        opts.append(" fixSOCLRS(-DfixLRS)="+fix_selectOneChoice_listRangeSize);
        opts.append(" fixComboboxLRS(-DfixLRS)="+fix_combobox_listRangeSize);
        opts.append(" -DtuneSOC="+fix_tune_tables);
        opts.append(" -DtuneCombobox="+fix_tune_combobx);
        opts.append(" -DfixLOVOnlyIfMaxExceeded="+m_fixlov_only_if_max_exceeded);
        opts.append(" -DfixViolations="+m_fixViolations);
        opts.append(" -DfixMultipleBc="+fix_MultipleBcUsages);
    ...
    private static void setSystemProperties() {
        ...
        String fixOnlyIfMax = System.getProperty("fixLOVOnlyIfMaxExceeded");
        if (!isEmpty(fixOnlyIfMax) && fixOnlyIfMax.trim().equals("true")) {
            m_fixlov_only_if_max_exceeded = true;
            String fixMaxThreshold = System.getProperty("fixMax");
            Integer fixThreshold = -1;
            if (!isEmpty(fixMaxThreshold)) {
                try {
                    fixThreshold = Integer.parseInt(fixMaxThreshold);
                    m_fixlov_Max = fixThreshold;
                } catch (Exception ex) {
                    ex.printStackTrace();
                    System.out.println("Invalid system property passed: fixMax has to be an integer");
                    System.exit(1);
                }
            }
        } else {
            m_fixlov_only_if_max_exceeded = false;
        }
        
        String strfixViolationOverride = System.getProperty("fixViolations");
        if(!isEmpty(strfixViolationOverride) && strfixViolationOverride.trim().equals("false"))        
              m_fixViolations=false;
        else if(!isEmpty(strfixViolationOverride) && strfixViolationOverride.trim().equals("true"))             
              m_fixViolations=true;
        
        
        String fixLrs = System.getProperty("fixListRangeSize");
        if(!isEmpty(fixLrs) && fixLrs.trim().equalsIgnoreCase("true"))
        {
            fix_tune_selectoneChoice=true;
            fix_combobox_listRangeSize=true;
        }
        else if(!isEmpty(fixLrs) && fixLrs.trim().equalsIgnoreCase("false"))
        {
            fix_tune_selectoneChoice=false;
            fix_combobox_listRangeSize=false;
        }
        
        //-DdoAde=true/false
        String doAdeOverRide = System.getProperty("doADE");
        if(!isEmpty(doAdeOverRide) && doAdeOverRide.trim().equals("false"))        
            bDoADE=false;
        else if(!isEmpty(doAdeOverRide) && doAdeOverRide.trim().equals("true"))             
            bDoADE=true;
      
        //-Dappend=true/false
        String appendOverRide = System.getProperty("append");
        if(!isEmpty(appendOverRide) && appendOverRide.trim().equals("false"))
            append=false;
        else if(!isEmpty(appendOverRide) && appendOverRide.trim().equals("true"))
            append=true;
      
        //-Dappend=true/false
        String debugOverRide = System.getProperty("debug");
        if(!isEmpty(debugOverRide) && debugOverRide.trim().equals("false"))
            bDebug=false;
        else if(!isEmpty(debugOverRide) && debugOverRide.trim().equals("true"))
            bDebug=true;
      
        String sysfixMultipleBc = System.getProperty("fixMultipleBc");
        if(!isEmpty(sysfixMultipleBc) && sysfixMultipleBc.trim().equals("false"))
            fix_MultipleBcUsages=false;
        else if(!isEmpty(sysfixMultipleBc) && sysfixMultipleBc.trim().equals("true"))
            fix_MultipleBcUsages=true;
      
        String sysfixSOC = System.getProperty("tuneSOC");
        if(!isEmpty(sysfixSOC) && sysfixSOC.trim().equals("false"))
            fix_tune_selectoneChoice=false;
        else if(!isEmpty(sysfixSOC) && sysfixSOC.trim().equals("true"))
            fix_tune_selectoneChoice=true;
        
        String sysFixCombo = System.getProperty("tuneCombobox");
        if(!isEmpty(sysFixCombo) && sysFixCombo.trim().equals("false"))
            fix_tune_combobx=false;
        else if(!isEmpty(sysFixCombo) && sysFixCombo.trim().equals("true"))
            fix_tune_combobx=true;
    }

    public static String getListOfSupportedOptions()
    {
        StringBuffer sb = new StringBuffer();
        sb.append(" -DfixViolations=true/false");
        sb.append(" -DdoADE=true/false");
        sb.append(" -DfixListRangeSize=true/false");
        sb.append(" -DfixMultipleBc=true/false");
        sb.append(" -DtuneSOC=true/false");
        sb.append(" -DtuneCombobox=true/false");
        sb.append(" -DfixLOVOnlyIfMaxExceeded=true/false");
        sb.append(" -DfixMax=Integer(used in conjunction with fixLOVOnlyIfMaxExceeded)");
        return sb.toString();
    }
    ...
    public static void main(String[] args) {
        try {
            ...
            writer = new BufferedWriter(new FileWriter("TableAndLOVTuning_LRG.csv"));
            writer.write("family,module,product,filename,label,Component Name,Component Id, AutoHeightRows,IteratorRangeSize," +
                         ...
                         "VODEfaultFetchSize_New,AM Package, VO Package, VO Instance, Fix Status\n");
            ...
            for (int j = 0; j < families.size(); j++) {
                ...
                if (pass1AnalysisPath.equals(sCrawlDir)) {
                    onlyreqiured_analysisRecords = allfin_analysisRecords;
                    onlyrequired_errorRecords = allfin_errorRecords;
                } else {
                    lovAnalyzer.crawlDirToFilterAnalysisOnly(sCrawlDir);
                }
                
                lovAnalyzer.writeOutputXml(onlyreqiured_analysisRecords, onlyrequired_errorRecords, "lovresults.xml");
                
                System.out.println("======== WRITING RESULTS TO SPREADSHEET ============");
                writeToSpreadsheet(onlyreqiured_analysisRecords, "Results");
     ...
     * @return
     */
    private static boolean isValidDirectory(String absDirPath) {
    ...
    //                return false;
        ...
        if(absFilePath.contains("/prj/components/projectsManagement") ||
            absFilePath.contains("/prj/components/projectsFinancialsSoa/projectManagement") ||
            absFilePath.contains("/prj/components/projectsFinancialsSoa/projectManagementControl") ||
            absFilePath.contains("/prj/components/projectsFinancialsSoa/resourceManagement") ||
            absFilePath.contains("/prj/components/projectsFinancialsEss/projectManagement") ||
            absFilePath.contains("/prj/components/projectsFinancialsEss/resourceManagement"))
            return false;
    ...
    private static void intializeSpreadsheet(String worksheetName) {
        ...
        m_analysisSheet.setColumnWidth(15, 30);
        ...
        m_analysisSheet.addCell(new Cell(new CellLocation("E1"), new CellValue("LovType")));
        // Column 6 -  autoHeightRows
        m_analysisSheet.addCell(new Cell(new CellLocation("F1"), new CellValue("AutoHeightRows")));
        // Column 7 - Iterator fetch size
        m_analysisSheet.addCell(new Cell(new CellLocation("G1"), new CellValue("IteratorRangeSize")));
        // Column 8 - New: Iterator fetch size
        m_analysisSheet.addCell(new Cell(new CellLocation("H1"), new CellValue("IteratorRangeSize.NewVal)")));
        // Column 9 - Iterator fetch size
        m_analysisSheet.addCell(new Cell(new CellLocation("I1"), new CellValue("ListRangeSize")));
        // Column 10 - New: Iterator fetch size
        m_analysisSheet.addCell(new Cell(new CellLocation("J1"), new CellValue("ListRangeSize.NewVal)")));
        // Column 11 - vousage fetch size
        m_analysisSheet.addCell(new Cell(new CellLocation("K1"), new CellValue("VOUsageFetchSize")));
        // Column 12 - vo default fetch size
        m_analysisSheet.addCell(new Cell(new CellLocation("L1"), new CellValue("VOusageFetchSize.NewVal")));
        // Column 13 - vnew vo usage fetch sizeo default fetch size
        m_analysisSheet.addCell(new Cell(new CellLocation("M1"), new CellValue("VODefaultFetchSize")));
        // Column 14 - vnew vo usage fetch sizeo default fetch size
        m_analysisSheet.addCell(new Cell(new CellLocation("N1"), new CellValue("VODefaultFetchSize.NewVal")));
        // Column 15 - vnew vo usage fetch sizeo default fetch size
        m_analysisSheet.addCell(new Cell(new CellLocation("O1"), new CellValue("RulesApplied")));
        // Column 16 - vnew vo usage fetch sizeo default fetch size
        m_analysisSheet.addCell(new Cell(new CellLocation("P1"), new CellValue("FixStatus")));
        // Column 17 - vnew vo usage fetch sizeo default fetch size
        m_analysisSheet.addCell(new Cell(new CellLocation("Q1"), new CellValue("fixStatusComment")));
    ...
    public static void writeComponentInSheetRow(int rowNumber, TuningAnalysisRecord record) {
        ...
        String lovTypeCellNumber = "E" + rowNumber;
        String autoHeightRowsCellNumber = "F" + rowNumber;
        String iteratorFetchSizeCellNumber = "G" + rowNumber;
        String New_iteratorFetchSizeCellNumber = "H" + rowNumber;
        String listRangeSizeCellNumber = "I" + rowNumber;
        String New_listRangeSizeCellNumber = "J" + rowNumber;
        String voUsageFetchSizeCellNumber = "K" + rowNumber;
        String New_voUsageFetchSizeCellNumber = "L" + rowNumber;
        String voDefaultFetchSizeCellNumber = "M" + rowNumber;
        String New_voDefaultFetchSizeCellNumber = "N" + rowNumber;
        String rulesAppliedCellNumber = "O" + rowNumber;
        String fixStatusesCellNumber = "P" + rowNumber;
        String fixCommentCellNumber = "Q" + rowNumber;
        ...
        try{
               ...
               if(fileName.contains("fusionapps/crm/components/sales/moo/" +
               ...
               if(record.getVoFullPackage().equals("oracle.apps.crmCommon.activities.activitiesService.view.TemplateTaskVO") &&
                   fileName.contains("fusionapps/crm/components/crmCommon/zmm/activities/publicUi/public_html/" +
                   "oracle/apps/crmCommon/activities/publicUi/page/activitiesTaskTemplateListViewPageLevel.jsff"))
                   return;
               
               if(record.getVoFullPackage().equals("oracle.apps.scm.doo.workbench.uiModel.view.FulfillLineTransientVO"))
                   return;
               
               if(record.getVoFullPackage().equals("oracle.apps.financials.receivables.receipts.manualBatches.uiModel.view.ReceiptWARTPBatchUIVO")
                   && f.getCanonicalPath().contains("fusionapps/fin/components/receivables/ar/receipts/manualBatches/ui/public_html/oracle/apps/financials/receivables/receipts/manualBatches/ui/page/ReceiptWABatches.jsff"))
                   return;
               
               if(record.getVoFullPackage().equals("oracle.apps.sales.salesForecasting.forecasts.forecastService.view.ForecastItemDetailVO")
                   && fileName.contains("fusionapps/crm/components/sales/zsf/forecasts/uiEndUser/public_html/oracle/apps/sales/salesForecasting/forecasts/ui/endUser/page/ForecastsTerritoryView.jsff"))
                   return;
               
               //Bug 14521035
               if(record.getVoFullPackage().equals("oracle.apps.contracts.coreAuthoring.salesAgreement.model.view.SalesAgreementLineVO")
                   && fileName.contains("fusionapps/crm/components/contractManagement/okc/coreAuthoring/salesAgreement/" + 
                   "ui/public_html/oracle/apps/contracts/coreAuthoring/salesAgreement/ui/page/SalesAgreementLineSummary.jsff"))
                   return;
               ...
               //Yao : Bug 16014718
               if(record.getVoFullPackage().equals("oracle.apps.cdm.foundation.parties.relationshipService.view.OrganizationContactVO") 
                  && fileName.contains("fusionapps/crm/components/crmCommon/hz/parties/uiFuse/public_html/oracle/apps/cdm/foundation/parties/ui/fuse/contacts/page/HzFuseContactList.jsff"))
                 return;
               if(record.getVoFullPackage().equals("oracle.apps.cdm.foundation.parties.relationshipService.view.OrganizationContactVO") 
                && fileName.contains("fusionapps/crm/components/crmCommon/hz/parties/publicUi/public_html/oracle/apps/cdm/foundation/parties/publicUi/relationships/page/HzCpuiOrgContactViewList.jsff"))
                 return;
               if(record.getVoFullPackage().equals("oracle.apps.cdm.foundation.parties.relationshipService.view.OrganizationContactVO") 
                && fileName.contains("fusionapps/crm/components/crmCommon/hz/parties/uiFuse/public_html/oracle/apps/cdm/foundation/parties/ui/fuse/contacts/page/HzFuseCustomerContactList.jsff"))
                 return;
               
               //Su: Bug 16415921
               if(record.getVoFullPackage().equals("oracle.apps.financials.revenueManagement.revenueCompliance.multipleElementArrangement.uiModel.view.ViewHeaderUIVO") 
                  && fileName.contains("fusionapps/fin/components/receivables/vrm/revenueCompliance/multipleElementArrangement/ui/public_html/oracle/apps/financials/revenueManagement/revenueCompliance/multipleElementArrangement/ui/page/MeaWorkArea.jsff"))
                 return;
               
               //Su: Bug 16745040
               if(record.getVoFullPackage().equals("oracle.apps.financials.payables.payments.singlePayments.uiModel.view.DisbursementHeaderVO")
                   && fileName.contains("fusionapps/fin/components/payables/ap/payments/singlePayments/ui/public_html/oracle/apps/financials/payables/payments/singlePayments/ui/page/ManagePaymentsPF.jsff"))
               return;
               if(record.getVoFullPackage().equals("oracle.apps.financials.payables.invoices.transactions.uiModel.view.ManageInvoiceVO")
                   && fileName.contains("fusionapps/fin/components/payables/ap/invoices/transactions/ui/public_html/oracle/apps/financials/payables/invoices/transactions/ui/page/ManageInvoicePF.jsff"))
               return;
            
            //Bug 17004168
            if(record.getVoFullPackage().equals("oracle.apps.financials.payables.invoices.transactions.uiModel.view.InvoiceDistributionVO")
                && fileName.contains("fusionapps/fin/components/payables/ap/invoices/transactions/ui/public_html/oracle/apps/financials/payables/invoices/transactions/ui/page/ManageDistributionPF.jsff"))
            return;
            
            //Bug 16894738
            if(record.getVoFullPackage().equals("oracle.apps.cdm.foundation.parties.personService.view.PersonDVO")
                && fileName.contains("fusionapps/crm/components/crmCommon/hz/parties/publicUi/public_html/oracle/apps/cdm/foundation/parties/publicUi/persons/page/HzCpuiContactPicker.jsff"))
            return;
            
            writer.write(FamilyModuleHelper.getFileNameInfo(f.getCanonicalPath()) + record.getComponentName() + "," + record.getComponentId() 
