...
public class RunLOVAnalysis {
    ...
    static Worksheet m_analysisSheet;
    static int m_currentrRow = 2;
     ...
     */
    static HashMap<String, ArrayList<TuningAnalysisRecord>> allfin_errorRecords = new HashMap<String, ArrayList<TuningAnalysisRecord>>();


    static HashMap<String, ArrayList<TuningAnalysisRecord>> onlyrequired_errorRecords =
     ...
     */
    static HashMap<String, ArrayList<TuningAnalysisRecord>> allfin_analysisRecords = new HashMap<String, ArrayList<TuningAnalysisRecord>>();

    /**
     * Map of all the anlaysis records for all of financials
     */
    static HashMap<String, ArrayList<TuningAnalysisRecord>> onlyreqiured_analysisRecords =
        new HashMap<String, ArrayList<TuningAnalysisRecord>>();
...
//        m_fixViolations=true;
    ...
    public static void main(String[] args) {
       ...
       if (args.length < 1  || isEmpty(args[0])) {
            System.out.println("Usage:  automateVOTuning.sh <path to your code in an open ade view> ["+getListOfSupportedOptions()+"]");
            System.exit(1);
        }
       ...
       /* if(sCrawlDir.contains("/LATEST")) {
        ...
        // Zeesha - added as per requests to avoid setting the listRangeSize params if they are already set. Only set them if the max is exceeded
        // also allowing for system overrides with fix violations - allows to run in analysis mode only. 
        // TODO: allow system override for supressing warnings.. 
        setSystemProperties();

        System.out.println("Directory to analyze:" + sCrawlDir + "\n");
        System.out.println("\n ===> Running with: "+getRunningWithParams()+"\n\n");
        
        errorLogger = initLogger();
        RunLOVAnalysis lovAnalyzer = new RunLOVAnalysis();      
        UiFileHelper.bDebug = bDebug;

        try {
            csvWriter = new BufferedWriter(new FileWriter("FetchSizes_scan.csv", append));
            if(!append)
               csvWriter.write("family,module,product,filename,label,Component Name,Component Id, AutoHeightRows,IteratorRangeSize," +
            ...
            System.out.println("Exception in pass1: " + ex.getMessage());
            ex.printStackTrace();
            System.exit(1);
        }
        ...
        if (m_fixViolations) {
            try {
                lovAnalyzer.scanAndMakeFixes(sCrawlDir, bDoADE);
            } catch (Exception ex) {
        ...
        String summary = lovAnalyzer.printStatistics(startDate);
        ...
        try {
            ...
            System.out.println("Exception occurred while writing xml: " + ex.getMessage());
            ex.printStackTrace();
        }
        ...
        try {
            ...
            System.out.println("Exception occurred while writing results to spreadsheet : " + ex.getMessage());
            ex.printStackTrace();
        }
        Date runDate = new Date();
       // Mail.sendMail("zeesha.currimbhoy@oracle.com", summary, "SCRIPT: LOVAnalysis Script Run at: " + runDate.toString());
    }
    ...
    public void fixPageDefIterators(TuningAnalysisRecord record, boolean bDoADE, HashMap<String, TuningAnalysisRecord> multipleIteratorFetchSizes) {
        ...
        try {
            ...
            if (multipleIteratorFetchSizes.containsKey(pageDefKey)) {
                ...
                //[ZEESHA] TODO: Find out how to handle status since we are now storing the value in a hashset
            ...
            while (listRecordsIter.hasNext()) {
                ...
                if (fixStatuses.contains(TuningConstants.FixStatus.STATUS_MULTIPLE_BC)) {
                    ...
                    if (multipleBcsFixed.containsKey(bcKey)) {
                        ...
                        // TODO: remove status_multiiple_bc
                    }
                }
    ...
    public static void writeComponentInSheetRow(int rowNumber, TuningAnalysisRecord record) {
        ...
        try{
           ...
           if(record.getAmFullPackage().equals("oracle.apps.customerCenter.infrastructure360.uiModel.manageContacts.applicationModule.ZcmManageContactsAM") 
            ...
            csvWriter.write(FamilyModuleHelper.getFileNameInfo(f.getCanonicalPath()) + record.getComponentName() + "," + record.getComponentId() 
