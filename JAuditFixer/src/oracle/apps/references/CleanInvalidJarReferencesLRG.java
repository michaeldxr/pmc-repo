...
public class CleanInvalidJarReferencesLRG {

    private static boolean ignoreHangingRefs = false;
    private static int m_jprSeen = 0;    /*Number of jprs seen*/
    private static int m_jprFixed = 0;  /*Number of jprs fixed*/
    private static int m_hangingRefsJprFixed = 0; /*Number of jprs fixed*/
    private static int m_nonExistentRefsJprFixed = 0;/*Number of jprs fixed*/
    private static int m_libraryFixed = 0;/*Number of library files fixed*/
    private static int m_librarySeen = 0; /*Number of library files fixed*/
    private static String m_diffFileName = "diff.txt"; /*Name of the diff file*/
    private static int releaseNum=0;
    
    private static HashSet<String> invalidJars = new HashSet<String>(); /*Hash set of all the invalid jars that are to be used as a comparison point for the jprs*/
    private static HashSet<String> crmInvalidJars = new HashSet<String>();
    
    private static BufferedWriter writer;
    private static ArrayList<String> listOfErrorMessages = new ArrayList<String>();
    private static final String issuetype = "InvalidJarReference";
    
    private HashSet<String> nonExistentJarsRemoved = new HashSet<String>();/*Hash set of all the invalid jars that are to be used as a comparison point for the jprs*/
    private AdeDiffHelper diffHelper = new AdeDiffHelper(m_diffFileName); /*Diff helper to write diff output to the file*/
    private boolean bDebug = false; /*debug flag - used to print sop statements when in debug mode*/
    private HashSet<String> libraryRefsRemoved = new HashSet<String>(); /*Set of invalid libraries removed in the entire scan*/
    private HashSet<String> librariesMovedToADF = new HashSet<String>(); /*Set of libraries moved to the adf library due to bad placement*/
    private boolean removeNonExistentJarRefs = true;

    public CleanInvalidJarReferencesLRG() {
        super();
    }
    ...
    private static void initializeCrmInvalidJars() {
     ...
     */
    private static void initializeInvalidJars() {
    ...
    public static void main(String[] args){
        try {
           ...
           // String series = "FUSIONAPPS_PT.V2MIB_LINUX.X64";
            ...
            CleanInvalidJarReferencesLRG invalidJarCleaner = new CleanInvalidJarReferencesLRG();
            ...
            for (int j = 0; j < families.size(); j++) {
                ...
                invalidJarCleaner.crawlDirectoryNew(viewRoot,sCrawlDir, false);                
    ...
    private void crawlDirectoryNew(String viewRoot, String path, boolean bDoADE) {
        ...
        try{
            ...
            ProcessBuilder pb = new ProcessBuilder("bash","-c","zcat " + viewRoot + 
                ".labellog.emd.gz | grep 'FILE " + path + "' | egrep 'ADF_Library_Dependencies.library|jpr'" +
            ...
            while((line = reader.readLine()) != null) {
                ...
                if (isValidJpr(line.trim())){
                    processJpr(viewRoot + line.trim(),bDoADE);
                    if (!ignoreHangingRefs)
                        removeHangingInvalidLibraryRefs(viewRoot + line.trim(), bDoADE);
    
                    if (removeNonExistentJarRefs) {
                        removeNonExistentLibraryRefs(viewRoot + line.trim(), bDoADE);
                    }
                }
                else if (isValidLibraryFile(line.trim())) 
                    processADFLibraryFile(viewRoot + line.trim(), bDoADE);
    ...
    public void removeNonExistentLibraryRefs(String jprFullPath, boolean bDoADE) {
        ...
        boolean bFixMade = false;
        if(jprFullPath.endsWith("EgoProtectedModelEffGen.jpr") || jprFullPath.endsWith("EgoProtectedModelEffUIGen.jpr"))
            return;
        try {
            ...
            for (int i = 0; i < classPathLen; i++) {
                ...
                for (int j = 0; j < urlNodeLen; j++) {
                    ...
                    if (!jarExists) {
                        ...
                        if(!urlPath.contains("/opmn") && !urlPath.contains("AdfExtnContentPublicModelCommon.jar") 
                           && !urlPath.contains("SvcAppCmmnCompNotesPublicModel_Common.jar")) 
            ...
            if (!isEmpty(nodeName) && nodeName.trim().equals("Item")) {
                ...
                if (!isEmpty(jarPath) && isInvalidJarRef(libraryFilePath, jarPath)) {
                    ...
                    try {
                        ...
                        File f = new File(libraryFilePath);
                        String canonicalPath = f.getCanonicalPath();
                        if(!jarPath.contains("AdfExtnContentPublicModelCommon.jar") && !jarPath.contains("SvcAppCmmnCompNotesPublicModel_Common.jar"))
...
//            return false;
    ...
    public boolean isValidLibraryFile(String absDirPath) {
        ...
        //Su: Bug 17814100
        if(releaseNum <= 8 && (absDirPath.contains("fusionapps/hcm/components/hcmCore/SuperWeb/ADF_Library_Dependencies.library")
                               || absDirPath.contains("fusionapps/hcm/components/hcmCoreSetup/SuperWeb/ADF_Library_Dependencies.library") 
                               || absDirPath.contains("fusionapps/hcm/components/hcmCore/locCN/person/data/ui/ADF_Library_Dependencies.library")))
            return false;
