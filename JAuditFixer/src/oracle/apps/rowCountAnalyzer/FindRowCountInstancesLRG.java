...
public class FindRowCountInstancesLRG {
    
    private static int m_javaCount = 0; //number of files processed
    private static int num_occurances = 0; //number of instances of rowCount
    private static int num_unused = 0; //number of instances where rowCount variable is never used
    private static int num_existence_checks = 0; //num of instances where rowCount is used as an existence check (or in any conditional expr)
    private static int num_logging = 0; //number of instances where rowCount variable is only used for logging    
    private static int num_iterations = 0; //number of instances when rowCount is used in iterations (for or while loops)
    private static int num_range = 0; //number of instances when rowCount is used for setting ranges
    private static int num_object_creation = 0; //number of instances when rowCount is used for object creation (new)
    private static int num_return_stmts = 0; //number of instances when rowCount is used to return a value
    ...
    private static final int STATUS_USAGE_UNKNOWN = 1;
    private static final int STATUS_USED_IN_ITERATION = 2;
    private static final int STATUS_USED_IN_OBJECT_CREATION = 3;
    private static final int STATUS_USED_IN_EXISTANCE_CHECK = 4;
    private static final int STATUS_USED_IN_RETURN = 5;
    private static final int STATUS_USED_IN_LOGGING = 6;
    private static final int STATUS_USED_IN_RANGE= 7;    
    private static final int STATUS_USED_IN_EXISTANCE_CHECK_COMPLEX = 8;
    
    private static HashMap<String,HashSet> exemptions = new HashMap<String,HashSet>();
    private static HashMap<String,ArrayList> exemptionLines = new HashMap<String,ArrayList>();
    
    private static final Pattern pRowCount = Pattern.compile("(\\w+)\\s*=\\s*.*(getEstimatedRowCount|estimatedRowCount|getCappedRowCount)");
    private static final Pattern pIteration = Pattern.compile("(for|while)\\s*\\(");
    private static final Pattern pObjectCreation = Pattern.compile("new\\s+");
    private static final Pattern pExistenceCheck = Pattern.compile("(==|!=|<=|>=|<|>)");
    private static final Pattern pReturnStmt = Pattern.compile("(return\\s+)|(return\\()");
    private static final Pattern pLogging = Pattern.compile("(print\\(|println\\(|write\\(|logMessage\\()");
    private static final Pattern pRange = Pattern.compile("setRangeSize");    
    private static final String issuetype = "SelectCountQuery";
    ...
    public static void main(String[] args) {
        try {
            ...
            writer = new BufferedWriter(new FileWriter("SelectCountQuery_LRG.csv"));
            writer.write("Family,Module,Product,Filename,Label, Line, Line#, Type of RowCount, Usage Type\n");

            generateExemptionList();
            FindRowCountInstancesLRG rowCountAnalyzer = new FindRowCountInstancesLRG();
  ...
  public void findViolationsInFile(File file, String fileContentsWithoutComments) throws Exception {
        ...
        try {
            do {
                ...
                Object[] lineAndNumLines = readLine(fileName);
                if (lineAndNumLines == null)
                    break;
                ...
                numOfOpenBraces +=
                        countNumberOfOpeningBracesInStr(line) - countNumberOfClosingBracesInStr(line);
                ...
                if (index != -1) {
                    outputLines.add(new Object[] { variableName,
                                                   FamilyModuleHelper.getFileNameInfo(canonicalPath) +
                                                   line.trim().replace(",",
                                                                       "<comma>") +
                                                   "," + lineNum + "," +
                                                   usageType +
                                                   testExpressionUsage(line,
                                                                       file.getName()) });
                    num_occurances++;
                } else
                    continue;
  ...
  private void writeToFile(String variableName, ArrayList outputLines, HashMap rowCountVariables) throws IOException {
      ...
      for(Iterator i = outputLines.iterator(); i.hasNext(); ) {
          ...
          if(varName != null && varName.equals(variableName)){
              writer.write(appendUsageType(variableName, rowCountVariables, line));
              i.remove();
              return;
          }
      }
  ...
  private static void removeExceptionCases() throws Exception{
      
    BufferedReader fileReader = new BufferedReader(new FileReader("SelectCountQuery_LRG.csv"));
    String line = null;
  ...
  private static boolean exemptionsContainsLine(String fileName, String lineNo, String line){
