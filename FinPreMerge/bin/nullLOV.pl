...
# $Header: fatools/opensource/jauditFixScripts/FinPreMerge/bin/nullLOV.pl /main/1 2011/01/13 12:31:55 hkaniven Exp $
use Cwd qw(getcwd);
use FileHandle;
use File::Find;
use Data::Dumper;
...
Purpose: Show NULL LOV in xml files
Syntax:
...
sub procDir{
  ...
  #Search for valid family components only
  if ($fdir =~ /fusionapps$/){
     if ($cur=~ /(atf|com|common|crm|fin|fscm|fs|hcm|ic|prd|prj|scm|\.)/){
        return;
    } else {
        $File::Find::prune = 1;
	print("***INFO:Not a family:$File::Find::name =pruned\n");
        return;
    }
  }

  if ($fdir =~ /fusionapps\/[a-z]+$/){
      if ($cur eq 'components' or $cur eq '.'){
	  return;
      }else{
          $File::Find::prune = 1;
	  #print("***INFO:not component: $File::Find::name =pruned\n");
          return;
      }
  }
...
sub procTransUnit{
    ...
    while ($curln=<$inFH>){
        #skip html comments
        ($curln =~ /^\s*<!--/) && next;
      	if ($curln =~ /_LOVUIHints_NullValueID/i){
            $numFound++;
            if ($numFound == 1){
